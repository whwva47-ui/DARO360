 ...
 *
 * Called by the webapp when it receives ?token= in the URL.
 * Validates the one-time token from extension-login,
 * marks it used, and returns the full operator profile.
 */
...
export async function POST(req: NextRequest) {
  ...
  if (new Date(record.expires_at) < new Date()) {
    await getSupabase().from('extension_tokens').delete().eq('id', record.id);
    return NextResponse.json({ error: 'Login link expired (5 min limit). Sign in again from the extension.' }, { status: 401, headers: h });
  }

  if (record.used) {
    return NextResponse.json({ error: 'This login link has already been used.' }, { status: 401, headers: h });
  }
