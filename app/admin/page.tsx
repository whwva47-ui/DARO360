      ...
      <div style={{maxWidth:'900px',margin:'0 auto'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'24px'}}>
          ...
          <button onClick={async()=>{
            ...
            const r = await fetch('/api/admin/digest', {headers:{'X-Admin-Key':'cic_admin_2026'}})
            const d = await r.json()
            if(d.success) setMsg('✓ Weekly report sent to whwva47@gmail.com')
            else setMsg('Error: ' + d.error)
          }} style={{background:'rgba(168,85,247,0.1)',border:'1px solid rgba(168,85,247,0.3)',color:'#a855f7',padding:'8px 16px',borderRadius:'6px',cursor:'pointer'}}>
            📧 Send Weekly Report
          </button>
        </div>
