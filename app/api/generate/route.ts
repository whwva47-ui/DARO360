...
function detectRequestType(message: string): string[] {
  ...
  // Angry or frustrated
  if (/\bangry\b|\bfurious\b|\bupset\b|\bannoy|\bfrustrat|\bwaste|\bscam|\bnot worth|\bstop|\bforget it|\bgive up|\bwhy bother|\bdisappoint|\bignor|\bghost/.test(lower)) types.push('angry')
  ...
  // Very short message (under 20 chars) — minimal response needed
  if (message.trim().length < 20) types.push('short_message')
  // Cold / unresponsive (alpha.date specific)
...
` : ""
  ...
  // Angry/frustrated handling
  const angryBlock = isAngry ? `
=== ANGRY OR FRUSTRATED MESSAGE ===
He seems upset, frustrated, or impatient. Do NOT match his energy or get defensive.
Always stay POLITE, KIND, and CALM.
Acknowledge his feelings genuinely — make him feel heard.
Gently redirect to something positive about your connection.
Use warmth to de-escalate. He should feel better after reading your reply, not more frustrated.
Never argue. Never be cold. Never dismiss his feelings.
=== END ANGRY HANDLING ===
` : ""
...
${locationNote}${tfBlock}${alphaBlock}${contactBlock}${meetupBlock}${angryBlock}${subscriptionBlock}${giftPhotoBlock}${questionBlock}${profileBlock}${historyBlock}${customBlock}
